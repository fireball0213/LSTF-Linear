...
class TsfKNN(MLForecastModel):
    ...
    def STL_search(self,x_stl, X_s, seq_len, pred_len):
        ...
        if self.approximate_knn == False:
            if self.trend=='STL':
                # distances = self._stl_modified_distance(x_stl.trend+x_stl.seasonal ,X_s_trend[:, :seq_len]+X_s_seasonal[:, :seq_len])
                distances = self._stl_modified_distance(x_stl.trend + x_stl.seasonal + x_stl.resid, X_s_trend[:, :seq_len] + X_s_seasonal[:, :seq_len]+X_s_resid[:,  :seq_len])
            else:
                # distances=self._stl_modified_distance(x_stl.seasonal+ x_stl.resid, X_s_seasonal[:, :seq_len]+X_s_resid[:, :seq_len])#使用季节性和残差计算距离
                distances = self._stl_modified_distance(x_stl.seasonal, X_s_seasonal[:, :seq_len])  # 使用季节性计算距离
            indices_of_smallest_k = np.argsort(distances)[:self.k]
        else:
            '''
            ...
            总会出现搜不到的情况，注意处理
            '''
            indices_of_smallest_k = self._lsh_search(x_stl.trend+x_stl.seasonal, self.k)
        ...
        if self.msas == 'MIMO':
            ...
            # neighbor_fore = X_s_seasonal[indices_of_smallest_k, seq_len]#使用季节性作为预测
            x_fore = np.mean(neighbor_fore, axis=0, keepdims=True)
            x_new_seasonal = np.concatenate((x_stl.seasonal[ 1:], x_fore), axis=0)
            x_new_resid = np.concatenate((x_stl.resid[1:], x_fore), axis=0)
            if pred_len == 1:
